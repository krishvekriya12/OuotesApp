...
import com.example.ouotesapp.Qoutes;
import com.example.ouotesapp.R;
...
public class CategoriesAdapter extends RecyclerView.Adapter<CategoriesAdapter.MyViewHolder>{
    ...
    public void onBindViewHolder(@NonNull CategoriesAdapter.MyViewHolder holder, int position) {
        ...
        holder.categoryName.setText(list2.getCategoriesName());
        holder.categoryImage.setImageResource(list2.getCategoryImg());

        holder.rootLayout.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
               Intent intent = new Intent(context, Qoutes.class);
               intent.putExtra("name",list2.getCategoriesName());
